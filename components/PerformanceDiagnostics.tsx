...
export const PerformanceDiagnostics: React.FC<PerformanceDiagnosticsProps> = ({
  ...
  const [experimentTargetGeneration, setExperimentTargetGeneration] = useState(350);
            ...
            <div className="rounded-xl border border-cyan-500/30 bg-cyan-950/10 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="max-w-3xl">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-gray-500">Holds Memory Discovery and the Full Pursuit framework fixed, then isolates the two remaining questions: whether stronger sustained Chaser pressure plus fair post-fall recovery is enough, and whether stricter contemporary cross-play/clean-tag selection is additionally needed. All runs share the same frozen benchmark bank and your current run is restored afterward.</p>
                </div>
              ...
              <div className="mt-3 grid gap-2 md:grid-cols-3 text-[10px]">
                <div className="rounded border border-gray-800 bg-black/20 px-3 py-2"><span className="font-semibold text-gray-300">A · Current Full Pursuit</span><div className="mt-1 text-gray-600">Exact previous control: Chaser 5.4 / 7.8, 32/s sprint drain, pressure starts, +6 closing bootstrap, early branches and the existing cross-play gate.</div></div>
                <div className="rounded border border-amber-500/20 bg-black/20 px-3 py-2"><span className="font-semibold text-amber-200">B · Stronger pursuit + fall protection</span><div className="mt-1 text-gray-600">Chaser 5.7 / 7.9 with 28/s sprint drain; Runner remains 5.0 / 7.5 and 24/s. Runner gets 900ms tag immunity after a fair fall respawn.</div></div>
                <div className="rounded border border-cyan-500/25 bg-black/20 px-3 py-2"><span className="font-semibold text-cyan-200">C · Strict clean cross-play</span><div className="mt-1 text-gray-600">B + direct retained-opponent engagement gating; showcase selection values clean pursuit tags and penalizes tags attributable to a recent Runner fall.</div></div>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-[180px_minmax(0,1fr)_auto] md:items-end">
                <label className="block">
                  ...
                  <input type="number" min={50} max={1500} step={50} disabled={architectureExperimentStatus.running} value={experimentTargetGeneration} onChange={e => setExperimentTargetGeneration(Math.max(50, Math.min(1500, Math.round(Number(e.target.value) || 350))))} className="mt-1 w-full rounded border border-gray-700 bg-gray-950 px-3 py-2 text-sm font-mono text-gray-200 disabled:opacity-50" />
                </label>
                <div>
                  <div className="flex justify-between gap-3 text-[10px] text-gray-500"><span className="truncate">{architectureExperimentStatus.running ? architectureExperimentStatus.label : 'Ready to compare 3 Full Pursuit refinements'}</span><span className="shrink-0">{architectureExperimentStatus.running ? `Gen ${architectureExperimentStatus.generation}/${architectureExperimentStatus.targetGeneration}` : `3 × ${experimentTargetGeneration} generations`}</span></div>
              ...
              <p className="mt-3 text-[10px] leading-relaxed text-gray-600">The default is 350 generations per condition (1,050 total). All three use the same elite self-play panel; only B/C change physics/fairness and only C adds the stricter contemporary/clean-tag selection layer. Cancelling discards temporary populations and restores the snapshotted run.</p>
            </div>
