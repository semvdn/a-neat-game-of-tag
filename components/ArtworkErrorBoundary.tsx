...
import { isExhibitionRequested, requestExhibitionRecovery } from '../services/runtimeRecovery';

type State = { error: Error | null; recoveryAttempted: boolean };

export class ArtworkErrorBoundary extends React.Component<React.PropsWithChildren, State> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo): void {
    console.error('Fatal artwork render error:', error, info);
    if (isExhibitionRequested()) {
      const recoveryAttempted = requestExhibitionRecovery(`react:${error.name}`);
  ...
  render(): React.ReactNode {
    ...
    const exhibition = isExhibitionRequested();
    return (
      <main className="flex h-screen w-screen items-center justify-center bg-gray-950 p-6 text-gray-200">
        <section className="max-w-lg rounded-xl border border-red-500/30 bg-gray-900 p-6 shadow-2xl">
          <h1 className="text-lg font-semibold text-red-300">Artwork runtime stopped</h1>
          <p className="mt-2 text-sm leading-relaxed text-gray-400">
            {exhibition && this.state.recoveryAttempted
              ? 'The exhibition runtime encountered an error and is reloading from the bundled showcase checkpoint.'
